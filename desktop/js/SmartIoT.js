 ...
 */
$('.exportDevices').on('click', function () {
    $('#div_alert').showAlert({message: '{{Listing }}', level: 'warning'});
    $.ajax({
        type: "POST",
        url: "plugins/SmartIoT/core/ajax/SmartIoT.ajax.php", 
        data: {
  ...
  if (!isset(_cmd)) {
   ...
   tr += '<span class="type" type="' + init(_cmd.type) + '">' + jeedom.cmd.availableType() + '</span>';
    ...
    if (is_numeric(_cmd.id)) {
     ...
     tr += '<a class="btn btn-success btn-sm cmdAction pull-left" data-action="add" style="margin-top:5px;"><i class="fa fa-plus-circle"></i> {{Commandes}}</a><br/><br/>'
   }
